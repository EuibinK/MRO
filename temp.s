...
#------------------------------------------------------------------------------------------------------------------
#
# Code originally written by Tim Goudge, adapted 2018-2022 by Mackenzie Day 
#
# Last Modified: February 2022
#
# NOT FOR DISTRIBUTION: Do not share without permission (daym@epss.ucla.edu)
#
# Code Description:
...
#		./DEM_UCLA_HiRISE-CTX-2022.s HIRISE_ID1 HIRISE_ID2 CTX_ID1 CTX_ID2 1 
#		
#		Here HIRISE_ID1 and HIRISE_ID2 are the HiRISE image names (e.g.,PSP_001882_1410), 
#			CTX_ID1 and CTX_ID2 are the CTX image names (e.g., P14_006699_2135), and
#			1 equals the resolution of the output HiRISE DEM in meters per pixel (mpp).
#
# To set up this code on a new machine, change the path names of the '_loc' variables (line ~60). 
#
#------------------------------------------------------------------------------------------------------------------

#Print working directory and set up folder structure for intermediate products:
...
#Set input variables: the four image names (HiRISE1 HiRISE 2 CTX1 CTX2) 
#Output DEM resolution (HiRISE) is set to 1 m/px. CTX DEM is 18 m/px. 
#NOTES:
...
#Set the project names as image1_image2 for both CTX and HiRISE
projname_h=${image1_h}_${image2_h}
projname_c=${image1_c}_${image2_c}

#Cumulative index is needed for both CTX and HiRISE. Up-to-date versions can be downloaded here by navigating to the 
#most recent orbit and then looking in the folder 'index' for cumindex.tab (CTX): 
...
#The MOLA SQL table (needed to match CTX DEM to MOLA elevation) is of the form: LON|LAT|ELEV|RADIUS|ORBIT

#Define location of HiRISE/CTX cumulative index tables, source for image download, and MOLA reference data: 
cumindex_loc_h=`echo /u/paige/raquel/dems/EDRCUMINDEX.tab`
main_path_h=`echo https://hirise-pds.lpl.arizona.edu/PDS`
cumindex_loc_c=`echo /u/paige/raquel/dems/cumindex.tab`
main_path_c=`echo http://pds-imaging.jpl.nasa.gov/data/mro/mars_reconnaissance_orbiter/ctx/`
mola_table_loc=`echo /u/paige/raquel/dems/mola`

#Set variable below to 1 to save out ellipsoid-referenced DEMs as well as areoid-referenced DEMs.
...
#Check that 2 .IMG files downloaded/are present in folder, then continue (Must have clean folders when starting) 
#TROUBLESHOOTING NOTE: This is a common failure point when starting a DEM after a previous attempt failed 
# Be sure to remove old .IMG files or this check will fail 
...
if [ ${img_count_c} -eq 2 ]; then
	...
	#Includes generation of GoodPixelMap which is created in the 'result' folder
	#Using default stereo settings. This is fine. 
	mkdir result
	...
	if [ ${num_mola_pts} -gt 100 ]; then
		...
		point2dem ${projname_c}_tied-trans_reference.tif -o temp -r mars --sinusoidal --proj-lon ${clon_c} --nodata -99999 --dem-spacing 18
...
#---------------------------------------------------HiRISE DEM-----------------------------------------------------
#Now produce the HiRISE DEM --> only do this if the CTX DEM finished 
if [ ${CTX_DEM_flag} -eq 1 ]; then
	...
	if [ ${img_count_1_h} -eq ${img_download_count_1_h} ] && [ ${img_count_2_h} -eq ${img_download_count_2_h} ]; then
		...
		#Call the stereo code
		#NOTE: THIS CODE USES THE DEFAULT PARAMETERS SET BY ASP. 
		#TO CHANGE, CREATE YOUR OWN STEREO.DEFAULT FILE IN THE WORKING DIRECTORY 
		mkdir result
...
if [ ${HiRISE_DEM_flag} -eq 1 ]; then
	...
	#	Use a max displacement of 2000 m and the IAU D_MARS datum
	# CHANGED BY DAY TO 500 m  
	pc_align --max-displacement 500 --datum D_Mars --save-transformed-source-points ${projname_c}_tied-trans_reference.tif ${projname_h}-PC.tif -o ${projname_h}_tied
	...
	point2dem ${projname_h}_tied-trans_source.tif -o temp -r mars --sinusoidal --proj-lon ${clon_h} --nodata -99999 --dem-spacing ${output_res_h}
	...
	#Now move the pc_align produced files
	mkdir PC_Align_Produced_Files/pdart 
...
if [ ${HiRISE_DEM_flag} -eq 1 ] && [ ${CTX_DEM_flag} -eq 1 ]; then
	...
	mv CTX_DEM/result/*GoodPixelMap* Final_Tied_DEMs/
	...
	mv HiRISE_DEM/result/*GoodPixelMap* Final_Tied_DEMs/
